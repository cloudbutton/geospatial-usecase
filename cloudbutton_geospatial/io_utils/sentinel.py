...
def download_unzip_transform_to_geotiff(product):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        band4 = glob.glob(f"{sentinel_product_dir[0:26].replace('1C', '2A')}*/GRANULE/*/IMG_DATA/R10m/*B04*")
        band4 = band4[0] if len(band4) == 1 else None
        band8 = glob.glob(f"{sentinel_product_dir[0:26].replace('1C', '2A')}*/GRANULE/*/IMG_DATA/R10m/*B08*")
        band8 = band8[0] if len(band8) == 1 else None
        band4_tiff_file = jp2_to_cog(band4)
        band8_tiff_file = jp2_to_cog(band8)
        upload_band_file(band4_tiff_file, product)
        upload_band_file(band8_tiff_file, product)
